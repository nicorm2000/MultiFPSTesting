        ...
        void RpcHealthStateChanged(int currentHealth, byte hittedPartID, AttackType attackType, uint attackerID)
        {
            ...
            if (!clientDead)
            {
                ...
                if (CurrentHealth <= 0)
                {
                    ...
                    Health attacker = GameManager.GetHealthInstance(attackerID);
                    if(attacker)
                        attacker.Client_KillConfirmation?.Invoke(hittedPartID, netId);
                ...
                if (ClientFrontend.ClientPlayerInstance && attackerID != netId && attackerID == ClientFrontend.ObservedCharacterNetID())
